...
namespace praxicloud.core.metrics.traceprovider
{
    ...
    /// </summary>
    public sealed class TraceMetricsProvider : SimpleMetricsProvider
    {
        #region Variables
        /// <summary>
        ...
        public TraceMetricsProvider(long reportingInterval, bool includeLabels) : base(reportingInterval, null)
        {
        ...
        {
            return Task.Run(() => Trace.TraceInformation($"{ _metricLabels[name] }{ name } = { TextOutputUtilities.GetValueText(value) }"));
        }
